...
-- This script inserts sample data for testing ads in feed and reels
-- ============================================================

-- Note: UUIDs are generated using gen_random_uuid() for PostgreSQL
-- If you're running this manually, you may need to adjust UUIDs
...
VALUES 
    -- Feed Ad Assets (Images) - Using Picsum and Unsplash for reliable placeholder images
...
-- SELECT COUNT(*) as campaign_count FROM campaigns WHERE status = 'ACTIVE';
-- SELECT COUNT(*) as ad_group_count FROM ad_groups WHERE status = 'ACTIVE';
-- SELECT COUNT(*) as ad_count FROM ads WHERE status = 'ACTIVE';
-- SELECT a.id, a.name, ag.placements FROM ads a JOIN ad_groups ag ON a.ad_group_id = ag.id WHERE a.status = 'ACTIVE';
